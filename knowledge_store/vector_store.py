...
def _get_collection():
    ...
    if _collection is None:
        ...
        # The pre-built index was created with ChromaDB's DefaultEmbeddingFunction
        # (all-MiniLM-L6-v2 sentence-transformers). Always use the same function
        # for queries so vector spaces match — using a different function (e.g.
        # Gemini) against the stored embeddings would return garbage results.
        from chromadb.utils.embedding_functions import DefaultEmbeddingFunction
        embedding_fn = DefaultEmbeddingFunction()
        logger.info("Vector store: using DefaultEmbeddingFunction (all-MiniLM-L6-v2)")
